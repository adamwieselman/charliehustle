...
public class HtmlExtractionService
{
   ...
   public BaseballSavantData extractBaseballSavantVariableTimeframeJson (String opponentHand, String batterPitcher, Integer daysBack, boolean hitQuality)
      throws IOException, UnirestException, InterruptedException
   {
      ...
      String hitQualityString = "";

      if(daysBack != 0) {
         greaterThanDate = startDate.getYear() + "-" + String.format("%02d", startDate.getMonthValue()) + "-" + String.format("%02d", startDate.getDayOfMonth());
      }

      System.out.println("adding " + batterPitcher);

      if(batterPitcher.equals("batter")){
         pitcherThrows = opponentHand;
      }else{
         batterStands = opponentHand;
      }

      if(hitQuality){
         hitQualityString = "6%7C5%7C";
      }

      //System.out.println("https://baseballsavant.mlb.com/statcast_search?hfPT=&hfAB=&hfGT=R%7C&hfPR=&hfZ=&stadium=&hfBBL=&hfNewZones=&hfPull=&hfC=&hfSea=2022%7C&hfSit=&player_type=" + batterPitcher + "&hfOuts=&opponent=&pitcher_throws=" + pitcherThrows + "&batter_stands=" + batterStands + "&hfSA=&game_date_gt=" + greaterThanDate + "&game_date_lt=&hfInfield=&team=&position=&hfOutfield=&hfRO=&home_road=&hfFlag=&hfBBT=&metric_1=&hfInn=&min_pitches=10&min_results=0&group_by=name&sort_col=pitches&player_event_sort=api_p_release_speed&sort_order=desc&min_pas=0&chk_stats_pa=on&chk_stats_k_percent=on&chk_stats_bb_percent=on&chk_stats_xba=on&chk_stats_xslg=on&chk_stats_xobp=on&chk_stats_xwoba=on&chk_stats_launch_speed=on&chk_stats_launch_angle=on#results");

      BaseballSavantData baseballSavantData = new BaseballSavantData();
      HttpClient client = HttpClient.newHttpClient();
      HttpRequest request = HttpRequest.newBuilder()
                                       .uri(URI.create("https://baseballsavant.mlb.com/statcast_search?hfPT=&hfAB=&hfGT=R%7C&hfPR=&hfZ=&stadium=&hfBBL=&hfNewZones=&hfPull=&hfC=&hfSea=2023%7C&hfSit=&player_type=" + batterPitcher + "&hfOuts=&opponent=&pitcher_throws=" + pitcherThrows + "&batter_stands=" + batterStands + "&hfSA=" + hitQualityString + "&game_date_gt=" + greaterThanDate + "&game_date_lt=&hfInfield=&team=&position=&hfOutfield=&hfRO=&home_road=&hfFlag=&hfBBT=&metric_1=&hfInn=&min_pitches=10&min_results=0&group_by=name&sort_col=pitches&player_event_sort=api_p_release_speed&sort_order=desc&min_pas=0&chk_stats_pa=on&chk_stats_k_percent=on&chk_stats_bb_percent=on&chk_stats_xba=on&chk_stats_xslg=on&chk_stats_xobp=on&chk_stats_xwoba=on&chk_stats_barrel_batted_rate=on&chk_stats_hard_hit_percent=on#results"))
                                       .build();
      ...
      Element baseballSavantBodyData = doc.getElementsByTag("tbody").first();
      Elements baseballSavantDataRows = baseballSavantBodyData.getElementsByTag("tr");

      for(int y=0; y < baseballSavantDataRows.size(); y+=2){
         Elements baseballSavantTrDataRows = baseballSavantDataRows.get(y).getElementsByClass("tr-data");
         BaseballPlayerSavant baseballPlayerSavant = new BaseballPlayerSavant();

         for(int x=0; x < baseballSavantTrDataRows.size(); ++x){
            if(x == 1){
               baseballPlayerSavant.setPlayerId(baseballSavantTrDataRows.get(x).id().substring(3));
               //System.out.println(baseballSavantTrDataRows.get(x).html());
            }else if(x > 4){
               String baseballSavantValue = baseballSavantTrDataRows.get(x).getElementsByTag("span").first().html();
               switch (x) {
                  case 5:
                     baseballPlayerSavant.setPlateAppearances(baseballSavantValue);
                     break;
                  case 6:
                     baseballPlayerSavant.setkPercent(baseballSavantValue);
                     break;
                  case 7:
                     baseballPlayerSavant.setBbPercent(baseballSavantValue);
                     break;
                  case 8:
                     baseballPlayerSavant.setExpectedBa(baseballSavantValue);
                     break;
                  case 9:
                     baseballPlayerSavant.setExpectedSlg(baseballSavantValue);
                     break;
                  case 10:
                     baseballPlayerSavant.setExpectedObp(baseballSavantValue);
                     break;
                  case 11:
                     baseballPlayerSavant.setExpectedWoba(baseballSavantValue);
                     break;
               }
            }
         }
         baseballSavantData.getBaseballPlayerSavantList().add(baseballPlayerSavant);
      }

      return baseballSavantData;
   }

   public CbsPlayerData extractCbsPlayerData (String batterPitcher)
      throws IOException, UnirestException, InterruptedException
   {
      ...
      if(batterPitcher.equals("batter")){
         System.out.println("Gathering batter cbs data");
         positions = "C:1B:2B:3B:SS:MI:CI:OF:DH";
      }else{
         System.out.println("Gathering pitcher cbs data");
         positions = "P";
      }

      CbsPlayerData cbsPlayerData = new CbsPlayerData();
      HttpClient client = HttpClient.newHttpClient();
      HttpRequest request = HttpRequest.newBuilder()
                                       ...
                                       .uri(URI.create("https://2007bkl.baseball.cbssports.com/stats/stats-main/all:" + positions + "/period-28:p/Stuff3?print_rows=9999"))
                                       .build();
      ...
      for(int x=1; x < 4; ++x) {
         ...
         for (Element cbsDataRow : cbsDataRows) {
            ...
            for (int y = 0; y < cbsTdDataRows.size(); ++y) {
               switch (y) {
                  case 1:
                     ...
                     if (!name.contains("W (") && !(name.startsWith("W") && name.length()==1)) {
            ...
            cbsPlayerData.getCbsPlayerList().add(cbsPlayer);
         }
      }
      System.out.println(cbsPlayerData.getCbsPlayerList().size());
      return cbsPlayerData;
   }
